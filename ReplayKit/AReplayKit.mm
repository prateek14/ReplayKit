...
extern "C"
{
    ...
    void rpbroadcast_LoadAndPresent()
    {
        ...
        ReplayKitLiveView* liveView = [[ReplayKitLiveViewModel Instance] liveView];
        if(liveView)
        {
            if([liveView isWindowShow])
                [liveView dissmissWindow];
            else
            {
                [liveView showWindow];
                liveView.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width / 2, [[UIScreen mainScreen] bounds].size.height / 2, 70, 70);
            }
        }
    }
